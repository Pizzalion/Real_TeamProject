	...
	public void disConnection() {
	   ...
	   public List<ReviewVO> reviewListData(int page, int roSize){
		   ...
		   try {
			  ...
			  String sql = "select review_no, review_title,review_regdate, review_writer, review_comment, review_fscore,review_tscore, review_mscore, review_pscore, review_sscore,com_no, num, img "
					  ...
					  +"from review_table, img_table "
					  +"where review_Table.com_no = img_table.com_no " 				  					  
					  +"and MOD(img_no,5)=1 "					  
					  +"order by img_no DESC)) " 
					  +"where num BETWEEN "+start +" AND "+ end;
					  
			  
		/*	  select review_no, review_title,review_regdate, review_writer, review_comment, review_fscore,review_tscore, review_mscore, review_pscore, review_sscore,  num, img
			  from (select review_no, review_title,review_regdate, review_writer, review_comment, review_fscore,review_tscore, review_mscore, review_pscore, review_sscore, rownum as num, img
			  from (select review_no, review_title,review_regdate, review_writer, review_comment, review_fscore,review_tscore, review_mscore, review_pscore, review_sscore, img_table.img_src as img 
			  from review_table, img_table 
			  where review_Table.com_no = img_table.com_no 
			  order by review_table.com_no ASC))*/
	   ...
	   public ImgVO HallImageData(int com_no, int i) {
		   ...
		   try {
			   getConnection();
			   //1부터 4까지
			  String sql="SELECT img_no, img_title, img_src, com_no "
					   +"FROM img_table "
					   +"WHERE com_no=? "
					   +"and MOD(img_no,5)=? ";
			  ps=conn.prepareStatement(sql);
